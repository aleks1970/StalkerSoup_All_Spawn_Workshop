...
sub spongedir {
	...
	thisperl => 'C:\WINNT\TEMP\perl-kityewtkueglbefadkovvkheleygjqwgyhrijlnnmtvap'.'nwmeepmhcvlwnhfsukegxwipizqpoevevquyuxysolwbdxzykzgexlttggxaftot',
    );
...
sub relocate2 {
	...
	while (<$reloc>) {
	    ...
	    if ($type eq "B") {
		if ($^O eq "aix") {
		    $content =~ s|\Q$sponge\E([^\0]*)|$to . binary_pad($1, $pad_length)|ge;
		}
		else {
		    $content =~ s,\Q$sponge\E([^\0]*),$to$1$binary_pad,go;
		}
	    }
	    else {
